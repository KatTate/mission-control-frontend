...
interface EditTaskModalProps {
  ...
  task: Task | null;
  onClose: () => void;
  onSubmit: (taskId: string, updates: EditTaskFormData) => Promise<void>;
}
...
export default function EditTaskModal({ isOpen, task, onClose, onSubmit }: EditTaskModalProps) {
  ...
  useEffect(() => {
    if (task) {
      setFormData({
        ...
        dueDate: task.dueDate?.split('T')[0] || '', // Convert ISO to YYYY-MM-DD
        tags: task.tags || [],
      });
    }
  }, [task]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Clean up form data
      const cleanData: EditTaskFormData = {
      ...
      await onSubmit(task.id, cleanData);
      onClose();
    } catch (error) {
